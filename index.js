    ...
    return new Promise((resolve) => {
        vinted.search(sub.url, false, false, {
            ...
            for (let item of itemsToSend) {
                ...
                const URL = `https://www.vinted.it/items/${item.id}`;
                    ...
                    client.channels.cache.get(sub.channelID)?.send({ embeds: [embed], components: [
                        new Discord.MessageActionRow()
                            .addComponents([
                                ...
                                new Discord.MessageButton()
                                    .setLabel('Paiement')
                                    .setURL(`https://www.vinted.it/transaction/buy/new?source_screen=item&transaction%5Bitem_id%5D=${item.id}`)
                                    .setEmoji('💳')
                                    .setStyle('LINK'),
                                new Discord.MessageButton()
                                    .setLabel('Négocier')
                                    .setURL(`https://www.vinted.it/items/${item.id}/want_it/new?button_name=receiver_id=${item.id}`)
                                    .setEmoji('💸')
                                    .setStyle('LINK'),
                               new Discord.MessageButton()
...
client.on('ready', () => {
    ...
    client.user.setActivity(`Cop Vinted 8€/mois`);
});
